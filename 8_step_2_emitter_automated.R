...
for (Lin in 1:4){
  Letter<-Letters[Lin]
  for (i in sample_names(get(paste0("ps_", Letter, "_A")))){
...
df2$Yellow_transmission<-rownames(df2) %in% tax_Y_s2b
df2$Blue_transmission<-rownames(df2) %in% tax_B_s2b
df2$Red_transmission<-rownames(df2) %in% tax_R_s2b
df2$Green_transmission<-rownames(df2) %in% tax_G_s2b
...
for(Lin in 1:4){
  ...
  Letter<-Letters[Lin]
  colour<-Colours[Lin]
  samples<-sample_names(get(paste0("ps_", Letter, "_A")))
  castes<-sample_data(get(paste0("ps_", Letter, "_A")))$Caste_composite
  meta_df<-data.frame(samples = samples, castes = castes)
  list_df[[Lin]]<-select(df3, c(paste0("Sample_", sample_names(get(paste0("ps_", Letter, "_A")))), paste0(Lineage, "_transmission")))
  cnames<-colnames(list_df[[Lin]])
  colnames(list_df[[Lin]])<-c(cnames[1:2], "Transmission")
  for (i in 1:nrow(list_df[[Lin]])){ # Get means of abundances per caste
    Queen_samples<-list_df[[Lin]][i, paste0("Sample_", meta_df[meta_df$castes=="Female Alate", "samples"])]
  ...
  # plot_W_clr[[Lin]]<-ggplot(data=list_df_W[[Lin]], aes(x=Mean_W_clr, fill=Transmission, alpha=0.6))+
  #   geom_density(outline.type = "lower")+scale_x_log10()+ guides(alpha=F, fill=F)+
  #   scale_fill_manual(values = c("black", colour))+
  #   ylab(NULL)+xlab("Strain abundance")
  # plot_L_clr[[Lin]]<-ggplot(data=list_df_L[[Lin]], aes(x=Mean_L_clr, fill=Transmission, alpha=0.6))+
  #   geom_density(outline.type = "lower")+scale_x_log10()+ guides(alpha=F, fill=F)+
  #   scale_fill_manual(values = c("black", colour))+
  #   ylab(NULL)+xlab("Strain abundance")
}
...
pdf("~/Desktop/R/Vertical_transmission_2020/final_figs/3_4_main/step_3_emitter.pdf",
    width = 10, height = 3, onefile = T)
grid.arrange(plot_Q_clr_3[[1]], plot_Q_clr_3[[2]], plot_Q_clr_3[[3]], plot_Q_clr_3[[4]], nrow = 1) # 10-3
dev.off()

# do.call(grid.arrange, plot_W)
# do.call(grid.arrange, plot_L)
...
for(Lin in 1:4){
  ...
  ps_3_g_sub<-subset_samples(ps_3_g, sample_data(ps_3_g)[,"Colony"]==Lineage &
                               sample_data(ps_3_g)[,"Caste_composite"]=="Female Alate")
...
do.call(grid.arrange, ps_Q[[1]])
do.call(grid.arrange, ps_Q[[2]])
do.call(grid.arrange, ps_Q[[3]])
do.call(grid.arrange, ps_Q[[4]])

pdf("~/Desktop/R/Vertical_transmission_2020/final_figs/3_4_supplementary/step_3_emitter_yellow.pdf",
    width = 14, height = 12, onefile = T)
do.call(grid.arrange, ps_Q[[1]]) # 14 - 12
dev.off()
pdf("~/Desktop/R/Vertical_transmission_2020/final_figs/3_4_supplementary/step_3_emitter_red.pdf",
    width = 14, height = 12, onefile = T)
do.call(grid.arrange, ps_Q[[2]])
dev.off()
pdf("~/Desktop/R/Vertical_transmission_2020/final_figs/3_4_supplementary/step_3_emitter_green.pdf",
    width = 14, height = 12, onefile = T)
do.call(grid.arrange, ps_Q[[3]])
dev.off()
pdf("~/Desktop/R/Vertical_transmission_2020/final_figs/3_4_supplementary/step_3_emitter_blue.pdf",
    width = 14, height = 12, onefile = T)
do.call(grid.arrange, ps_Q[[4]])
dev.off()
...
# do.call(grid.arrange, ps_W[[1]])
# do.call(grid.arrange, ps_W[[2]])
# do.call(grid.arrange, ps_W[[3]])
# do.call(grid.arrange, ps_W[[4]])
# 
# do.call(grid.arrange, ps_L[[1]])
# do.call(grid.arrange, ps_L[[2]])
# do.call(grid.arrange, ps_L[[3]])
# do.call(grid.arrange, ps_L[[4]])

#### Statistics
library(nlme)
m0_Q<-data.frame(matrix(ncol=1,nrow=length(list_df_Q)))
# m0_W<-data.frame(matrix(ncol=1,nrow=length(list_df_W)))
# m0_L<-data.frame(matrix(ncol=1,nrow=length(list_df_L)))
# colnames(m0)<-c("kw_p")
...
df_Q_new<-rbind(list_df_Q[[1]][,3:19],
                list_df_Q[[2]][,3:19],
                list_df_Q[[3]][,3:19],
                list_df_Q[[4]][,3:19])
